...
function MovieGrid({ title, data, currentPage, totalPages, loading, error, onPageChange }) {
    
    if (loading) return <p className="p-4 text-white">Caricamento {title.toLowerCase()}...</p>;
    if (error) return <p className="error p-4 text-red-500">{error}</p>;
    if (!data || data.length === 0) return <p className="p-4 text-white">Nessun risultato trovato per {title.toLowerCase()}.</p>;
