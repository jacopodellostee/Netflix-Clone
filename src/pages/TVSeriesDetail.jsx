...
export default function TVSeriesDetail() {
  ...
  async function getTVSeriesDetail() {
    const response = await fetch(`${BASE_URL}/tv/${id}?language=en-EN&append_to_response=credits`, {
      headers: {
    ...
    if (!response.ok) {
      throw new Error("API call error. The TV series may not exist or the ID is incorrect.");
    }
  ...
  if (isLoading) {
    return <div className='bg-black text-white min-h-screen flex items-center justify-center'><p className="text-xl">Loading TV series...</p></div>;
  }
  ...
  if (!tvSeries.id) {
    return <div className='bg-black text-white min-h-screen flex items-center justify-center'><p className="text-xl">TV series details not available.</p></div>;
  }
        ...
        <div className="relative z-10 h-full flex items-end px-6 md:px-12 pb-20">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap items-center gap-3 md:gap-6 mb-6 text-sm md:text-base">
              ...
              <span>{formatRuntime(tvSeries.episode_run_time)}/episode</span>
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="w-full">
            ...
            <div className="flex flex-wrap justify-center items-center gap-3 mb-4 text-sm">
              <span className="text-yellow-400 font-semibold flex items-center">
                <span className="text-xl mr-1 items-center">★</span> {Math.round(tvSeries.vote_average * 10) / 10} / 10
              </span>
              {tvSeries.first_air_date && (
                <span>{new Date(tvSeries.first_air_date).getFullYear()}</span>
              )}
      ...
      <div className="px-4 md:px-12 py-2 md:py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 text-sm md:text-base">
          ...
          <div className="space-y-4">
            <div>
              <p className="text-gray-400 font-medium">Creators:</p>
              <p className="font-bold text-lg">{getCreators(tvSeries.credits)}</p>
            </div>
            <div>
              <p className="text-gray-400 font-medium">Main Character:</p>
              <p className="font-medium">{getMainCast(tvSeries.credits)}</p>
            </div>
            <div>
              <p className="text-gray-400 font-medium">Number of Seasons:</p>
          ...
          <div className="space-y-4">
            ...
            <div>
              <p className="text-gray-400 font-medium">Last Episode Date:</p>
              ...
              <div key={season.id} className="flex flex-col md:flex-row bg-black rounded-lg overflow-hidden shadow-lg transition duration-300">
                ...
                <div className="p-4 md:p-6 flex flex-col justify-center">
                  ...
                  {season.overview ? (
                    ...
                    <p className="text-gray-500 italic">No synopsis available for this season.</p>
                  )}
                </div>
              </div>
            ))}
